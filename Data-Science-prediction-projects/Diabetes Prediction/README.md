...
- After hyperparameter tuning was performed on the Decision Tree and Random Forest, it was found that the F1 values for both algorithms decreased, while the recall values actually increased
- In the Random Forest model, the highest F1-score was obtained compared to other algorithms. In addition, the relatively balanced precision and recall values indicate that this model is suitable for predicting diabetes status in patients, both those with and without diabetes.
- After hyperparameter tuning, the SVM and Random Forest models had lower overall performance values but showed high recall values. This indicates that both models are capable of capturing most patients with diabetes, making them more suitable for use in the initial screening stage.
- In the SVM and Random Forest models (after hyperparameter tuning), although they had lower overall performance values, they showed high recall values. This indicates that both models are capable of detecting most patients with diabetes, making them more suitable for use in the initial screening stage.
